...
export const authService = {
  ...
  async checkEmailExists(email: string): Promise<{ exists: boolean; error: AuthError | null }> {
    try {
      // Query the profiles table to check if email exists
      // This is cleaner than attempting a dummy login
      ...
      if (error && error.code !== 'PGRST116') {
        ...
        return { exists: false, error: null }; // Allow signup attempt on error
      }
      ...
      // On any error, allow signup attempt (Supabase will handle validation)
  ...
  // Verify email with OTP (6-digit code from email)
  async verifyEmailOTP(email: string, token: string): Promise<{ user: AuthUser | null; error: AuthError | null }> {
    try {
      const { data, error } = await supabase.auth.verifyOtp({
        email,
        token,
